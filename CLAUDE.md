...
평가 범위 = **기술 부합도 매핑 + 연구개발 부합도 검토 의견(4단계 분류)**: 별표7의2(국가전략 30%+α)를 먼저 검토한
뒤, 별표7(신성장 20%+α)를 검토하고, 평가기준.md(v1.2) 기반으로 ✅/🔵/🟢/⚪ 4단계 자가진단 의견을 함께 제시한다.
공통·필수요건 게이트(연구소 신고·인력 전담·설비 용도)와 STEP1·2 적격성 게이트는 여전히 본 도구 범위 외이며,
부합도 분류는 5대 판단기준·개발단계·제외활동 정의에 근거한 **자가진단 의견**이다.
...
lib/assess.ts  assess()          (엔진 디스패처)
   ...
   ├─ Bedrock → AnthropicBedrock + IAM Role/SigV4 (HD현대 LLM Gateway)
   │            ↑ 위 둘은 runMessagesAssessment() 공통 헬퍼 공유
...
3개 엔진을 커버한다. Claude/Bedrock은 `tool.input_schema`로 전달하고, Gemini는 `toGeminiSchema()` 재귀
변환기를 거쳐 `responseSchema`로 변환된다. **결과 필드를 추가·제거할 때 이 한 곳만 수정하면 전 엔진에 동시 반영된다.**

`friendlyError()`가 크레딧 부족·잘못된 키·레이트리밋·Bedrock 자격증명 오류를 사용자 친화 메시지로 변환한다.

**기본값**: `engine=gemini` / `model=gemini-2.5-flash-lite` (Claude: `claude-sonnet-4-6`, Bedrock: `claude-sonnet-4.6`) / `기준범위=조선추출`. 엔진을 Claude/Bedrock 외로 전환하면 `별표전문`은 `조선추출`로 자동 강등된다.

**Bedrock 엔진 특이사항**: 사용자 API 키 불요 — 서버의 AWS 자격증명 체인(IAM Role · `AWS_PROFILE`)으로 자동 인증. 필수 env: `ANTHROPIC_BEDROCK_URL` / 선택: `AWS_REGION`(기본 `ap-northeast-2`) · `ANTHROPIC_BEDROCK_TOKEN`. HD현대 LLM Gateway 전용(가이드: aws-guide.hd.com/s/llmgw).
...
| 함수 | 역할 |
|---|---|
| `lookupAnnexLabel(id)` | ID → `[별표7의2 6-아-1)] 환경친화적 첨단 선박의 운송ㆍ추진 기술` 원문 반환 |
| `lookupAnnexDescription(id)` | ID → 시행령 원문 설명 반환 (호 단위 ID면 해당 호 본문만). 후처리에서 `기술의설명` 주입에 사용 |
| `nationalStrategyIds()` | `국-` 접두 리프 ID 목록 + `"해당없음"` — 국가전략매핑 배열 enum 후보 |
| `newGrowthIds()` | `신-` 접두 리프 ID 목록 + `"해당없음"` — 신성장매핑 배열 enum 후보 |
| `enabledAnnexIds()` | 전체 리프 ID + `"해당없음"` — 국/신 통합 enum 후보 |
...
**서버 후처리 신뢰 경계**: `postProcessResult()`가 ① AI 반환 `매핑항목명`을 카탈로그 원문으로 **덮어쓰고**, ② `기술의설명`(`lookupAnnexDescription`)을 시행령 원문으로 **주입**하며, ③ `_기준메타`(`getCatalogMeta`)를 **첨부**한다.
UI에 표시되는 항목명·설명은 AI 출력이 아니라 항상 시행령 원문이다.
...
| `평가기준.md` | `assess.ts`가 로드 — 연구개발 부합도 검토 카드용 (5대 기준·4단계 분류·제외활동·경계영역·분쟁사례) |
| `기술참조표.md` | 현재 `assess.ts`가 로드하지 않음 (레거시·읽기 전용) |

`기술참조표.md`는 참조 문서로만 보존한다 — 수정 금지. `평가기준.md`는 가이드라인 개정 시 갱신한다.
...
**결과/입력 타입 단일 소스**: `types/index.ts` — 모든 한글 인터페이스(`AssessmentResult`, `AssessmentInput`, `매핑항목`, `연구개발부합도검토`, `부합도분류` 등). 결과 필드를 추가·제거할 때 `RESULT_SCHEMA`(`lib/assess.ts`)와 함께 갱신.
